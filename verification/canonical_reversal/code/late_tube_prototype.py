...
if result is None:result=dict(status='FEASIBLE IN FLOATING POINT; directed-rounding audit still required',terminal=19500*dt,final_radius=r)
result['runtime_seconds']=time.monotonic()-t0
out=root/'outputs/crossing-closure';(out/'LATE_TUBE_PROTOTYPE_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
if history:
 with (out/'LATE_TUBE_PROTOTYPE_PROFILE.csv').open('w') as f:
