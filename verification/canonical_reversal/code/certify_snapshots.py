...
import argparse
ap=argparse.ArgumentParser();ap.add_argument('--data',type=Path,default=Path('outputs/crossing-closure/canonical_snapshot_states.npz'));a=ap.parse_args();z=np.load(a.data);mp.iv.dps=60
...
for t,th in zip(z['times'],z['theta']):
 ...
 rows.append(dict(time=float(t),E_center_lower=scal(E.lo()),E_center_upper=scal(E.hi()),E_ball_lower=scal(lo),E_ball_upper=scal(hi),parameter_norm_ball_upper=scal(P),probe_error_Lipschitz_upper=scal(K)))
...
result=dict(status='STATIC SNAPSHOT BALLS ENCLOSED; TRAJECTORY REACHABILITY SEPARATE',radius='0.000035',snapshots=rows,central_first_gap_lower=float(gap1),central_rebound_lower=float(gap2),sector_angular_halfwidth='0.000001',sector_first_gap_lower=float(a1),sector_rebound_lower=float(a2),input_sha256=hashlib.sha256(a.data.read_bytes()).hexdigest())
(a.data.parent/'SNAPSHOT_CERTIFICATE.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
