...
parser=argparse.ArgumentParser();parser.add_argument('--reference',type=Path,default=Path('work/crossing-closure/canonical-reference'));parser.add_argument('--output',type=Path,default=Path('outputs/crossing-closure'));parser.add_argument('--workers',type=int,default=4);args=parser.parse_args()
X=I(np.load(args.reference/'X.npy'));th=np.load(args.reference/'theta.npy',mmap_mode='r');fv=np.load(args.reference/'velocity.npy',mmap_mode='r');N=len(X.c);h=th.shape[1]//2
...
def step(task):
 j,Rbar=task
 a0=I(th[j]);a1=I(th[j+1]);f0=I(fv[j]);f1=I(fv[j+1])
...
if __name__=='__main__':
 r=1e-6;history=[];started=time.monotonic()
 with (args.output/'LATE_TUBE_PROTOTYPE_PROFILE.csv').open() as f:
  prototype=list(csv.DictReader(f))
 tasks=[(int(row['step'])-1,float(row['radius'])*1.5+2e-6) for row in prototype]
 try:
  ...
  result=dict(status='LOCAL FLOW ENCLOSED FROM A BALL AT THE t=3.4 NUMERICAL CENTER; INITIALIZED REACHABILITY NOT CERTIFIED',initial_radius=1e-6,initial_reference_index=17000,final_reference_index=19500,dt='1/5000',final_radius_upper=float(r),inside_static_witness_ball=bool(r<3.5e-5))
 except Exception as err:
  result=dict(status='VALIDATION FAILED',error=repr(err),last_completed_step=history[-1]['step'] if history else None)
 result['runtime_seconds']=time.monotonic()-started
 args.output.mkdir(exist_ok=True,parents=True);(args.output/'LOCAL_FLOW_CERTIFICATE.json').write_text(json.dumps(result,indent=2)+'\n')
 if history:
  with (args.output/'LOCAL_FLOW_ENCLOSURE.csv').open('w') as f:
