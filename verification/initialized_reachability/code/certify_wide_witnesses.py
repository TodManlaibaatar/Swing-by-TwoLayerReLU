...
parser.add_argument('--data',type=Path,default=Path('inputs/canonical_witness_states.npz'))
...
for ti,time in enumerate(data['times']):
 R=up(2e-5 if time<3.5 else 4.6e-5)
 ...
 for cluster,sl in [('all',slice(None)),('strong',slice(0,N//2)),('weak',slice(N//2,N))]:
  ...
  for kind in ['Q_G','full_rate']:
   ...
   results.append(dict(time=float(time),radius_upper=float(R),cluster=cluster,quantity=kind,center_lower=scal(q.lo()),center_upper=scal(q.hi()),smooth_budget_upper=scal(smooth),gate_budget_upper=scal(gate),ball_lower=scal(lo),ball_upper=scal(hi),probe_margin_lower=probe_margin))
result=dict(status='STATIC WITNESS BALLS ENCLOSED; INITIALIZED TRAJECTORY REACHABILITY NOT CERTIFIED',arithmetic='Float64 midpoint-radius intervals; dot gamma_k bounds; outward nextafter sums/products; interval trigonometry at 60 decimal digits. Assumes standard IEEE round-to-nearest basic arithmetic, sqrt, and dot accumulation without unsafe fast-math.',static_radius_by_time={'3.4':'0.00002','3.9':'0.000046'},results=results,geometry=geometry,inputs={args.data.name:hashlib.sha256(args.data.read_bytes()).hexdigest()},versions=dict(python=platform.python_version(),numpy=np.__version__,mpmath=mp.__version__),not_claimed=['A true initialized flow reaches either ball','A gradient-flow crossing has been certified','High-probability iid initialization guarantee'])
